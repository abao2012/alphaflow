...
│                                                  │
│  ┌──────────┐  ┌──────────┐  ┌───────────────┐  │
│  │ Scoring  │  │ Emerging │  │  Candidate    │  │
│  │ Service  │  │ Detector │  │  Diagnostics  │  │
│  └────┬─────┘  └────┬─────┘  └───────┬───────┘  │
│       │              │                │           │
│  ┌────┴──────────────┴────────────────┴──────┐   │
│  │         Trend Feature Extractor           │   │
│  │         Breadth Feature Extractor         │   │
│  │         Quality Score Engine              │   │
│  └──────────────────┬────────────────────────┘   │
│                     │                            │
│  ┌──────────────────┴────────────────────────┐   │
│  │         QMT Connector (xtdata)            │   │
│  └───────────────────────────────────────────┘   │
│                                                  │
│  ┌───────────────────────────────────────────┐   │
...
AlphaFlow 面向**日更级趋势交易者**，而非日内交易者。日线数据足够捕捉趋势的形成和转折，同时避免日内噪音干扰判断。

### 为什么不用 Level2 数据？

Level2 数据需要付费权限且数据量巨大。AlphaFlow 基于日线 K 线、分钟 K 线、成交额和广度数据构建，降低了使用门槛。

### 为什么需要 QMT？

QMT（迅投 QMT）是国内券商常用的量化交易终端，提供免费的 `xtdata` 行情接口。AlphaFlow 通过 QMT 获取实时行情和历史数据，无需额外购买数据源。

### PostgreSQL 缓存层

QMT 的 xtdata 接口在批量查询时有性能瓶颈。PostgreSQL 缓存层将板块成分、个股信息、日线和分钟线数据持久化，显著减少重复查询。**不装 PostgreSQL 也能运行**，只是每次请求都会走 QMT 实时查询，速度较慢。
